...
def get_counts(histograms,avg_weight_histograms):
	...
	for ih,h in enumerate(histograms):		  
		...
		weighted_counts.append(unweighted_count.Clone("weighted count"))
		unweighted_count.Divide(avg_weight_histograms[ih])
		count_histograms.append(unweighted_count)
	return count_histograms, weighted_counts

def histograms_to_lists_by_bin(aliases,unweighted_count_histograms,avg_weight_histograms, weighted_count):
	counts_by_bins = {}
	unc_by_bins = {}
	weight_by_bins = {}
	weighted_count_bins = {}
	for b in range(2,7):
		...
		weighted_count_bins[b - 1] = pd.Series([h.GetBinContent(b) for h in weighted_count], index=[a[0] if a[1]==False else a[0]+","+str(a[2]) for a in aliases])
	return (counts_by_bins,unc_by_bins,weight_by_bins,weighted_count_bins)

def make_dataframes(aliases,counts_by_bins,unc_by_bins,weight_by_bins,weighted_count_bins):
	...
	print weights
	print pd.DataFrame(weighted_count_bins)
...
def main(options):
	...
	unweighted_count_histograms, weighted_count = get_counts(histograms,avg_weight_histograms)
	#stat_unc_histograms = get_stat_unc()
	print "Converting histograms to dictionaries of lists ..."
	counts_by_bins,unc_by_bins,weight_by_bins,weighted_count_bins = histograms_to_lists_by_bin(get_aliases(options.samples),unweighted_count_histograms,avg_weight_histograms, weighted_count)
	print "Making dataframes and printing them ..."
	dataframes = make_dataframes(get_aliases(options.samples),counts_by_bins,unc_by_bins,weight_by_bins,weighted_count_bins)
